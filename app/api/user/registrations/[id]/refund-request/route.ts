...
export async function POST(
  ...
  // Tell the organiser in-app. Best-effort: the refund flag is what matters, so
  // a notification failure must never fail the athlete's request.
  const organiserUserId = registration.event.organiser?.userId;
  if (organiserUserId) {
    try {
      await prisma.userNotification.create({
        data: {
          userId: organiserUserId,
          type: "ORGANISER_REFUND_REQUEST",
